...
namespace WaveBox.Service.Services {
    ...
    public class NatService : IService {
        // Logger
        private static readonly WaveBox.Core.Logging.ILog logger = WaveBox.Core.Logging.LogManager.GetLogger(typeof(NatService));
        ...
        public bool Start() {
            ...
            // Hook into the events so you know when a router has been detected
        ...
        public bool Stop() {
            ...
            if ((object)Device != null && Status == NatStatus.PortForwardedSuccessfully) {
                ...
                try {
                    int port = Injection.Get<IServerSettings>().Port;
                    Device.DeletePortMapAsync(new Mapping(Protocol.Tcp, port, port)).GetAwaiter().GetResult();
                } catch (Exception e) {
