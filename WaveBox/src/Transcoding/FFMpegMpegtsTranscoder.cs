...
namespace WaveBox.Transcoding
{
	public class FFMpegMpegtsTranscoder : VideoTranscoder
	{
		...
		protected override string GenerateArguments(uint abitrate, uint vbitrate, uint width, uint height)
		{
			...
			return " -ss " + OffsetSeconds + " -t " + LengthSeconds + " -i \"" + Item.FilePath + "\" -async 1 -b " + vbitrate + " -s " + width + "x" + height + " -ar 44100 -ab " + abitrate + " -ac 2 -v 0 -f mpegts -vcodec libx264 -preset superfast -strict experimental -acodec aac -threads 0 " + OutputPath;
