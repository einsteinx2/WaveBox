...
namespace WaveBox.Core.Model {
    public class Song : MediaItem {
        ...
        [JsonPropertyName("artistName"), IgnoreWrite]
        ...
        [JsonPropertyName("albumName"), IgnoreWrite]
        ...
        public override string GroupingName { get { return SongName; } }

        public Song() {
        }

        public override void InsertMediaItem() {
            Injection.Get<ISongRepository>().InsertSong(this, true);

            // Update art relationships
            Injection.Get<IArtRepository>().UpdateArtItemRelationship(ArtId, ItemId, true);
            Injection.Get<IArtRepository>().UpdateArtItemRelationship(ArtId, AlbumId, true);
